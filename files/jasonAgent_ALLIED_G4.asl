...
+!perform_injury_action 
    <- 
        ?shooting_to_me(Bullets2)[source(M)];
        if (M == "Manager") {
            ?my_health(Health2);
            NewHealth2 = Health2 + Bullets2;
            -+my_health(NewHealth2);
        }
.
...
+!init
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR init GOES HERE.")}
        ...
        if (MN == "A1") {
            .my_team("AXIS", E);
            .concat("shooting_to_me(", 98, ")", ContentSTM);
            .send_msg_with_conversation_id(E, tell, ContentSTM, "STM");
        }
   .
